...
> v1.2 → v1.3：新增「對話氣泡多型態」 — type / persistence / interaction 三維度擴充（thought / persistent / choice 等）；M2.5 階段插入。詳見 [對話氣泡類型](對話氣泡類型.md)。
...
兩套起始人格選自使用者既有素材（`角色素材/人物設定-*.{docx,png}`）：

| 內部 slug | 顯示名 | 素材 | 內容交付時機 |
|---|---|---|---|
| `haiyin` | 海音 | 角色素材/人物設定-海音.docx + 2 png | M4 開始前 |
| `liss` | 莉絲 | 角色素材/人物設定-莉絲.docx + 1 png | M4 開始前 |
...
對話氣泡採三維設計，**任意組合都成立**。完整論述見 [對話氣泡類型](對話氣泡類型.md)。
...
- **靜態圖素材來源**（自繪 / 委託 / AI 生圖）— 走到 P5a 前確認；既有 角色素材/*.png 是否可作為起點。

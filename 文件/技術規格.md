...
> 版本：v1.1（對應 設計需求.md v1.3）
> 確認日期：2026-05-08
> 涵蓋階段：P1–P7（自用主線）+ M2.5（氣泡多型態）
>
> v1.0 → v1.1：dialogues.json schema 擴充（type / persistence / interaction）；新增 IPC `dialogue:choice-selected`。詳見 [對話氣泡類型](對話氣泡類型.md)。

設計需求.md 描述「**做什麼、為什麼**」。本規格書描述「**怎麼做**」 — 模組責任、資料模型、介面契約、事件流程、錯誤處理。
...
| 文件 | 焦點 | 何時看 |
|---|---|---|
| 設計需求.md | 需求與決策 | 評估範圍、決定是否啟用某功能 |
| **技術規格.md** | **技術契約** | **寫程式時當作 source of truth** |
| 里程碑計畫.md | 時程與交付 | 排工作、驗收 |
| 三方視角分析.md | 商業/體驗評估 | 決策前回顧 |
| 隱私分析.md | 隱私威脅與緩解 | 設計隱私功能、商業化合規評估 |
...
- 上層：[設計需求](設計需求.md)
- 同層：[里程碑計畫](里程碑計畫.md)、[隱私分析](隱私分析.md)
